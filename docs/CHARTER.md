...
> **The canonical charter lives in the EISCORE repo at
> `Documentation/Web/EISWEB_CHARTER.md`, with the `WEB-` ticket ledger in
> `todo.md § EISWeb revival`. On any divergence, the EISCORE copy wins.**
...
| **D7** | Ticket prefix `WEB-`; ledger lives in EISCORE `todo.md`. WG- numbering untouched. |
