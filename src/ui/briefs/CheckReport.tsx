...
const VERDICT_STYLE = {
  ok: 'border-petrol/40 dark:border-petrol-dark bg-petrol-wash dark:bg-petrol-tint text-petrol-ink dark:text-petrol-light',
  warn: 'border-amber/50 dark:border-amber-dark bg-amber-wash dark:bg-amber-tint text-amber-ink dark:text-amber-light',
  bad: 'border-rust/50 dark:border-rust-dark bg-rust-wash dark:bg-rust-tint text-rust-dark dark:text-rust-light',
} as const;
...
const FINDING_STYLE: Record<BriefFinding['severity'], string> = {
  BLOCKER: 'border-rust/50 dark:border-rust-dark bg-rust-wash dark:bg-rust-tint text-rust-dark dark:text-rust-light',
  GAP: 'border-amber/50 dark:border-amber-dark bg-amber-wash dark:bg-amber-tint text-amber-ink dark:text-amber-light',
  NOTE: 'border-dust-200 dark:border-dust-700 bg-dust-0 dark:bg-dust-800 text-dust-600 dark:text-dust-300',
};
...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <h4 className="text-xs font-semibold uppercase tracking-wider text-dust-600 dark:text-dust-400">{children}</h4>
  );
}
...
function ResolutionRow({ kind, asked, resolved, extra }: {
  ...
  return (
    <tr className="border-t border-dust-200/70 dark:border-dust-800">
      <td className="py-0.5 pr-3 text-xs text-dust-600 dark:text-dust-400">{kind}</td>
      <td className="py-0.5 pr-3 text-sm text-dust-600 dark:text-dust-300">{asked || <span className="text-dust-600 dark:text-dust-400">(unstated)</span>}</td>
      <td className="py-0.5 pr-2 text-dust-600 dark:text-dust-400">&rarr;</td>
      <td className="py-0.5 pr-3 font-mono text-sm">
        {resolved !== null && resolved !== '' ? (
          <span className="text-petrol-ink dark:text-petrol-light">{resolved}</span>
        ) : (
          <span className="text-rust-dark dark:text-rust-light">UNRESOLVED</span>
        )}
      </td>
      <td className="py-0.5 text-xs text-dust-600 dark:text-dust-400">{extra ?? ''}</td>
    </tr>
  );
}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {counts && (
          <span className="text-xs text-dust-600 dark:text-dust-400">
            <span className={counts.blocker > 0 ? 'font-semibold text-rust-dark dark:text-rust-light' : ''}>
              {counts.blocker} blocker{counts.blocker === 1 ? '' : 's'}
            </span>
            {' · '}
            <span className={counts.gap > 0 ? 'font-semibold text-amber-ink dark:text-amber-light' : ''}>
        ...
        <span className="font-mono text-xs text-dust-600 dark:text-dust-400">exit {check.exitCode}</span>
            ...
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                {(r.pieces ?? []).map((p) => {
                  ...
                  return (
                    <tr key={p.piece} className="border-t border-dust-200/70 dark:border-dust-800 align-top">
                      <td className="py-1 pr-3 font-mono text-xs text-dust-900 dark:text-dust-100 whitespace-nowrap">
                        {p.piece}
                        {!p.consumed && (
                          <span className="ml-1 rounded border border-rust/50 dark:border-rust-dark bg-rust-wash dark:bg-rust-tint px-1 text-[10px] text-rust-dark dark:text-rust-light">
                            NO CONSUMER
                          </span>
                        )}
                      </td>
                      {familyStated && (
                        <td className="py-1 pr-3 text-xs whitespace-nowrap">
                          {p.family_hop === 1 ? (
                            <span className="text-petrol-ink dark:text-petrol-light">{p.in_family} in {cityStyle?.family}</span>
                          ) : p.family_hop != null ? (
                            <span className="text-amber-ink dark:text-amber-light">
                              {p.in_family} via {p.family_hop_name} (hop {p.family_hop})
                            </span>
                          ) : (
                            <span className="text-rust-dark dark:text-rust-light">0 in chain</span>
                          )}
                        </td>
                      )}
                      <td
                        className={`py-1 pr-3 text-right font-mono text-xs ${missing ? 'font-semibold text-rust-dark dark:text-rust-light' : thin ? 'text-amber-ink dark:text-amber-light' : 'text-dust-600 dark:text-dust-300'}`}
                      >
                        {p.have}
                        {missing ? ' — MISSING' : thin ? ' — thin' : ''}
                      </td>
                      <td className="py-1 pr-3 font-mono text-xs text-dust-600 dark:text-dust-300">
                        {p.styles.length > 0
                          ? p.styles.map(([st, n]) => `${st}(${n})`).join(', ')
                          : '—'}
                      </td>
                      <td className="py-1 pr-3 font-mono text-xs text-dust-600 dark:text-dust-400 whitespace-nowrap">
                        {p.consumers.length > 0 ? p.consumers.join('/') : '—'}
                      </td>
                      <td className="py-1 pr-3 text-xs text-dust-600 dark:text-dust-300">
                        {p.candidates > 0 && p.packs.length > 0 ? (
                          ...
                          <span className="text-dust-600 dark:text-dust-400">none in project (new art)</span>
                        ) : (
                          '—'
                        )}
                      </td>
                      <td className="py-1 text-xs text-dust-600 dark:text-dust-300">
                        ...
                        {p.family_note && (
                          <div className="mt-0.5 font-mono text-[11px] text-amber-ink dark:text-amber-light">
                            !! {p.family_note}
                          </div>
                        )}
                      </td>
          ...
          <ul className="space-y-0.5 text-xs">
            {(r.structure_coverage ?? []).map((sc) => (
              <li key={sc.structure} className="font-mono">
                <span className="text-dust-900 dark:text-dust-100">{sc.structure}</span>{' '}
                {sc.assets.length > 0 ? (
                  <span className="text-dust-600 dark:text-dust-300">{sc.assets.join(', ')}</span>
                ) : (
                  <span className="text-rust-dark dark:text-rust-light">no registered assets</span>
                )}
              </li>
            ))}
          </ul>
            ...
            <ul className="space-y-1.5">
              {items.map((f, i) => (
                <li key={i} className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                  ...
                  <span className="font-mono text-xs text-dust-600 dark:text-dust-400">[{f.section}]</span>
                  <span className="w-full text-sm leading-snug text-dust-600 dark:text-dust-300 sm:w-auto sm:flex-1">
                    {f.detail}
                    {f.fix && (
                      <span className="block text-xs text-dust-600 dark:text-dust-400">&rarr; {f.fix}</span>
                    )}
                  </span>
                </li>
              ))}
