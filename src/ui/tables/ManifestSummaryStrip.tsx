  ...
  return (
    <div className="panel flex flex-wrap items-center gap-x-4 gap-y-2 px-3 py-2 text-sm tabular-nums">
      ...
      {CLASSIFICATION_ORDER.map((c) => {
        ...
        return (
          <span key={c} className="flex items-center gap-1.5 text-dust-600 dark:text-dust-300">
      ...
      >
        <span className="font-mono font-semibold">{rowLossCount}</span> losing rows on import
      </span>
    </div>
  );
}
